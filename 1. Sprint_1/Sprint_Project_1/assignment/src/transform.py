...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = orders[orders["date"].dt.year == 2017]
    ...
    order_purchase_ammount_per_date = (
        filtered_dates.groupby(filtered_dates["date"].dt.date)
        .size()
    ...
    result_df = DataFrame()
    result_df["order_count"] = order_purchase_ammount_per_date["order_count"]
    result_df["date"] = pd.to_datetime(order_purchase_ammount_per_date["date"])
